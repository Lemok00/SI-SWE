...
if __name__ == '__main__':
    ...
    ''' Testing '''
    def hiding_process(vector, factor):
        with torch.no_grad():
            style = model.mapping(factor)
            container_image = model.generator(vector, style)
        return container_image

    def extracting_process(container_image, factor, t_max):
        with torch.no_grad():
            recovered_vector = torch.rand(size=(args.batch_size, model.vector_dim)).to(device) * 2 - 1
            style = model.mapping(factor)
    ...
    for i in tqdm(range(math.ceil(args.test_num / args.batch_size)),desc='Testing'):
        ...
        with torch.no_grad():
            container_image = hiding_process(vector, factor)
            for attack, intensities in attack_and_intensity_list:
                for intensity in intensities:
                    attacked_image = attack.attack(container_image.clone(), intensity)
                    recovered_vector = extracting_process(attacked_image, factor, t_max)
                    recovered_message = vector_to_message(
                        recovered_vector.view(recovered_vector.shape[0], -1), sigma=sigma)
