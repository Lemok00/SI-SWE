...
class ReferenceAttention(nn.Module):
    def __init__(self, dim):
        ...
        self.learnable_lambda = nn.Parameter(torch.zeros(1))

        self.K_conv = nn.Sequential(nn.Conv2d(in_channels=self.dim, out_channels=self.dim // 8,
                                              kernel_size=(1, 1)))
        self.Q_conv = nn.Sequential(nn.Conv2d(in_channels=self.dim, out_channels=self.dim // 8,
                                              kernel_size=(1, 1)))
        self.V_conv = nn.Sequential(nn.Conv2d(in_channels=self.dim, out_channels=self.dim,
                                              kernel_size=(1, 1)))

        self.softmax = nn.Softmax(dim=-1)

    def forward(self, primary_tensor, reference_tensor):
        b, c, h, w = primary_tensor.shape
        assert primary_tensor.shape[0] == reference_tensor.shape[0]

        key = self.K_conv(reference_tensor).view(b, -1, h * w)  # BxCxN
        query = self.Q_conv(reference_tensor).view(b, -1, h * w).permute(0, 2, 1)  # BxNxC
        energy = torch.bmm(query, key)  # BxNxN
        attention = self.softmax(energy)
        value = self.V_conv(primary_tensor).view(b, -1, h * w)  # BxCxN
        out = torch.bmm(value, attention.permute(0, 2, 1)).view(b, c, h, w)

        return primary_tensor + self.learnable_lambda * out
